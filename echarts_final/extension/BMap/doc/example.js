...
function getdetail(index ,tracedatainout){
    ...
    for(var index1 in tracedata){
        ...
        var inflag = true;
        //console.log(newsecarr);
        while(counte<newsecarr.length-1){
            var aimid ='';
            var nowid ='';
            aimid  = newsecarr[counte]["DEST_SITE_ID"];
            nowid = newsecarr[counte+1]["SITE_ID"];
...
//**************************************
    ...
    $.ajax({
        ...
        type: "post",        //type：(string)请求方式，POST或GET
        dataType: "json",    //dataType：(string)预期返回的数据类型。xml,html,json,text等
        url: "../../../data/tinyserverdata.json",  //url：(string)发送请求的地址，可以是服务器页面也可以是WebService动作。
        success: function (msg) {
            //console.log(msg);
    ...
    var serverid = new Set();

    for(var se in serverdata){
        //alert(serverdata[se]["startId"]);
        serverid.add(serverdata[se]["startId"]);
        serverid.add(serverdata[se]["endId"]);
    }
    //去重的存储了所有的地点的id
    ...
    for(var sdf of serverid){
        var key_set = localdic[sdf];
        ...
        for(var sindex in serverdata){
            ...
            if(serverdata[sindex]["startId"]===ttid){
                ...
                try{
                    ...
                    red = red+50*abnormalindex;
                    green = green-50*abnormalindex;
                    if(red>255){
                        red = 255;
                    }
                    if(green<0){
...
//*******************************************************************
    ...
    function (echarts, BMapExtension) {
        ...
        function eConsole(param){
            ...
            //alert(idindex);
            var siteclickid = toptenid[idindex];
            //alert(siteclickid);
            //获取id，发送ajax请求，
            //1表示out
            //2表示in
            //10.10.0.91:8080/atcal/siteinfo?siteId=15265&size=20&tag=1
            var requin = "10.10.0.91:8080/atcal/siteinfo?siteId="+siteclickid+"&size=5&tag=2";
            var requout= "10.10.0.91:8080/atcal/siteinfo?siteId="+siteclickid+"&size=5&tag=1";
            ...
            $.ajax({
                ...
                type: "post",        //type：(string)请求方式，POST或GET
                dataType: "json",    //dataType：(string)预期返回的数据类型。xml,html,json,text等
                url: "../../../data/informat.json",  //url：(string)发送请求的地址，可以是服务器页面也可以是WebService动作。
            ...
            $.ajax({
                ...
                type: "post",        //type：(string)请求方式，POST或GET
                dataType: "json",    //dataType：(string)预期返回的数据类型。xml,html,json,text等
                url: "../../../data/outformat.json",  //url：(string)发送请求的地址，可以是服务器页面也可以是WebService动作。
